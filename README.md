...
Phase 1 provides a read-only monitoring dashboard powered by Streamlit.
The requester now writes a live JSON snapshot to `app/dashboard-state.json`,
and the dashboard renders worker, job, and recent-result status from that file.
...
# Terminal 1: requester publishes state snapshots
node request-compute.js
...
- The dashboard is intentionally read-only for Phase 1.
- Refresh the browser to pull new snapshot data.
- You can override snapshot path with `DASHBOARD_STATE_PATH=/path/to/file.json`.

## Streamlit Dashboard (Phase 2)

Phase 2 adds live auto-refresh and basic action controls from the sidebar.
The Streamlit app appends commands to `app/dashboard-commands.jsonl`, and
the requester polls and executes them using the same command handler as CLI input.

### New controls

- Auto-refresh toggle + interval slider
- Queue `run <code>` tasks
- Queue `shell --timeout <ms> <command>` tasks
- Quick buttons for `status` and `workers`

### Optional overrides

- `DASHBOARD_STATE_PATH=/path/to/dashboard-state.json`
- `DASHBOARD_COMMANDS_PATH=/path/to/dashboard-commands.jsonl`
