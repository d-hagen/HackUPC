...
import { earnTaskPoints, formatPointLedger } from "./economy.js";

const workerId = `worker-${crypto.randomUUID().slice(0, 8)}`;
const storePath = `./store-${workerId}`;
const IDLE_TIMEOUT = 120000; // leave after 120s with no tasks
const ALLOW_SHELL =
  process.env.ALLOW_SHELL === "1" || process.env.ALLOW_SHELL === "true";
...
console.log(
  `Shell execution: ${ALLOW_SHELL ? "ENABLED" : "DISABLED (set ALLOW_SHELL=1 to enable)"}\n`,
);
...
function open(s) {
  return s.get("view", { valueEncoding: "json" });
}
...
function resetIdleTimer() {
  ...
  idleTimer = setTimeout(() => {
    if (base && !searching) {
      console.log(
        `[~] Idle for ${IDLE_TIMEOUT / 1000}s, leaving ${currentRequester}…`,
      );
...
async function joinRequester(requesterId, autobaseKey, conn) {
  ...
  resetIdleTimer();
  console.log(
    `[~] Connected to ${requesterId}, waiting for authorization...\n`,
  );
}
...
async function processTasks() {
  ...
  for (let i = 0; i < base.view.length; i++) {
    ...
    for (let j = 0; j < base.view.length; j++) {
      ...
      if (e.type === "result" && e.taskId === entry.id) {
        hasResult = true;
        break;
      }
    }
    ...
    try {
      ...
      if (entry.taskType === "shell" && output) {
        ...
        console.log(
          `[<] Done in ${elapsed}ms | exit=${output.exitCode} | ${stdoutPreview} | ${formatPointLedger(loadReputation())}`,
        );
        if (output.timedOut) console.log(`    [!] Process timed out`);
      } else {
        const preview = JSON.stringify(output ?? null).slice(0, 80);
        console.log(
          `[<] Done in ${elapsed}ms | ${preview} | ${formatPointLedger(loadReputation())}`,
        );
      }
      console.log(
        `    (${tasksDone} for this requester, ${totalTasksDone} total)\n`,
      );
    } catch (err) {
      await base.append({
        type: "result",
  ...
  if (didWork) {
    ...
    for (let i = 0; i < base.view.length; i++) {
      ...
      if (entry.taskType === "shell" ? !ALLOW_SHELL || !entry.cmd : !entry.code)
        continue;
      if (entry.assignedTo && entry.assignedTo !== workerId) continue;
    ...
    if (!hasRemainingTasks) {
      ...
      if (betterOption) {
        console.log(
          "[~] All tasks done here, another requester has work — moving on",
        );
        leaveCurrentRequester();
      }
    }
...
function pickBestRequester() {
  ...
  if (candidates.length > 0) {
    ...
    console.log(
      `[~] Found ${pick.id} with ${pick.info.pendingTasks} pending task(s)${scoreStr}`,
    );
    if (candidates.length > 1) {
      console.log(
        `    (${candidates.length - 1} other requester(s) available, picked highest reputation)`,
      );
    }
      ...
      if (msg.type === "advertise" && msg.role === "requester") {
        ...
        if (searching) {
          if (msg.pendingTasks >= 0) {
            ...
            console.log(
              `[~] Found ${msg.requesterId} (${msg.pendingTasks} pending tasks${scoreStr})`,
            );
